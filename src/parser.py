...
def parse_definitions(vcd_file) -> dict:
    ...
    def parse_var(line_str : str) -> dict:
        ...
        line_dict['type'] = line_list[1]
        line_dict['size'] = line_list[2]
        line_dict['symbol'] = line_list[3]
        line_dict['name'] = line_list[4]
        return line_dict
    ...
    def parse_scope(vcd_file, scope_name : str) -> dict:
        symbol_table = dict()
        symbol_table[scope_name] = []
        while True:
            line = vcd_file.readline()
            if line.startswith("$upscope $end"):
                break
            if line.startswith("$scope"):
                symbol_table.update(parse_scope(vcd_file, line.split()[2]))
            if line.startswith("$var"):
                symbol_table[scope_name].append(parse_var(line))
        return symbol_table
...
def symbol_map(main_table : dict) -> dict:
    symbol_table = dict()
    for scope in main_table:
        if scope:
            if type(main_table[scope]) == list:
                for symbol in main_table[scope]:
                    symbol_table[symbol['symbol']] = symbol['name']
            else:
                symbol_table.update(symbol_map(main_table[scope]))
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame.from_dict(data, orient="index").astype('string')
    df.replace("", np.nan, inplace=True)
    df.fillna(method="ffill", inplace=True) 
    
    df.to_csv("output_filled.csv")
